document.addEventListener('DOMContentLoaded', () => {
  ...
  // --- Modal Checkout ---
  function openCheckout() { renderCartItems(); checkoutModal.classList.remove('hidden'); document.body.style.overflow='hidden'; }
  function closeCheckoutModal() { checkoutModal.classList.add('hidden'); document.body.style.overflow='auto'; }
  ...
  function showConfirmPopup(orderText, finalAction){
    ...
    overlay.innerHTML=`<div class="bg-white p-6 rounded-xl shadow-lg max-w-md w-full text-center">
      ...
      <div class="text-left bg-gray-50 border rounded-lg p-3 mb-4 text-sm text-gray-800 overflow-y-auto max-h-48">${orderText.replace(/\n/g,"<br>")}</div>
  ...
  function openWhatsApp(orderText){
    window.open(`https://api.whatsapp.com/send?phone=${SELLER_PHONE}&text=${encodeURIComponent(orderText)}`,'_blank');
  }
  ...
  checkoutForm.addEventListener('submit', (e)=>{
    ...
    cart.forEach((item,i)=>{
      text += `${i+1}. ${item.title.substring(0,25)}... (${item.qty}x) - Rp ${(item.price*item.qty).toLocaleString('id-ID')}\n`;
    });
  ...
  // --- Info pembayaran saat metode dipilih ---
  paymentMethod.addEventListener('change', ()=>{
    ...
    } else paymentInfo.classList.add('hidden');
  });
